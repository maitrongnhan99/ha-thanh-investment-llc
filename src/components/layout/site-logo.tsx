...
function LogoMark({ className }: { className?: string }) {
  return (
    ...
    >
      <g
        fill="currentColor"
        transform="translate(32 32) scale(0.078) translate(-389 -350)"
      >
...
export function SiteLogo() {
  return (
    <span className="flex items-center gap-3">
      <LogoMark className="text-primary size-9 sm:size-11" />
      <span className="text-primary flex flex-col leading-none">
        <span className="font-sans text-[1.05rem] font-black tracking-tight sm:text-[1.35rem]">
          HATHANH
        </span>
        <span className="text-muted-foreground mt-0.5 font-sans text-[0.7rem] font-medium tracking-[0.18em] sm:mt-1 sm:text-[0.85rem]">
          INVESTMENT
        </span>
      </span>
